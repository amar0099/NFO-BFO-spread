# ─────────────────────────────────────────────
# auto_token.py
# Fully automated Fyers login using direct API
# calls — no browser, no headless method needed
# ─────────────────────────────────────────────
...
URL_VALIDATE     = BASE_URL_2 + "/validate-authcode"
...
def generate_token():
    ...
    try:
        # Extract APP_ID (remove -100 suffix)
        ...
        r1_data = r1.json()
        if r1_data.get("s") != "ok":
            return None, f"Step 1 failed: {r1_data}"
        ...
        r2_data = r2.json()
        if r2_data.get("s") != "ok":
            return None, f"Step 2 TOTP failed: {r2_data}"
        ...
        r3_data = r3.json()
        if r3_data.get("s") != "ok":
            return None, f"Step 3 PIN failed: {r3_data}"
        ...
        r4 = requests.post(URL_TOKEN, json={
            ...
            "response_type": "code",
            "create_cookie": True
        }, headers={"Authorization": f"Bearer {access_token_trade}"})
        r4_data = r4.json()
        if r4_data.get("s") != "ok":
            return None, f"Step 4 auth code failed: {r4_data}"

        auth_code = parse_qs(urlparse(r4_data["Url"]).query).get("auth_code", [None])[0]
        if not auth_code:
