...
def generate_token(client_id, secret_key, username, pin, totp_key):
    ...
    try:
        ...
        # Fyers may return "Url" or "url" depending on API version
        redirect_url = r4d.get("Url") or r4d.get("url") or r4d.get("data", {}).get("url", "")
        if not redirect_url:
            return None, f"Step 4: no redirect URL in response: {r4d}"
        auth_code = parse_qs(urlparse(redirect_url).query).get("auth_code", [None])[0]
        if not auth_code:
            return None, f"Step 4: no auth_code in redirect URL: {redirect_url}"
