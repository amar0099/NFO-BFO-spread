...
def get_shared_token():
    """Generates token once per server process — shared across ALL browser sessions.
    Returns (token_or_None, error_or_None) tuple so failures are cached too
    and don't trigger a retry storm on every expiry-dropdown render."""
    ...
    try:
        ...
        pass
    # Auto-generate via TOTP (single attempt — no blocking sleeps)
    ...
    if token:
        try:
            ...
            pass
        return token, None
    return None, error  # cache the failure — stops retry storm
...
if st.button("🔄 Refresh Token", key="refresh_token_nav", help="Clear cached token and re-authenticate"):
    ...
    fetch_expiries_for.clear()
    # Also delete stale token file so fresh login is attempted
    try:
...
if not _tok:
    st.error(f"❌ Login failed: {_tok_err}. Wait ~60s for Fyers rate limit to reset, then click **Refresh Token**.")
    st.stop()
