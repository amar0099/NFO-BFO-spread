...
def generate_token(client_id, secret_key, username, pin, totp_key):
    ...
    try:
        ...
        # Fyers API v3: auth code may come as redirect URL (old) or directly in data.auth (new)
        data = r4d.get("data", {})
        auth_code = (
            data.get("auth")                                                         # new format
            or parse_qs(urlparse(r4d.get("Url", "")).query).get("auth_code", [None])[0]   # old format
            or parse_qs(urlparse(data.get("url", "")).query).get("auth_code", [None])[0]  # fallback
        )
        if not auth_code:
