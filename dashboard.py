...
def generate_token():
    ...
    try:
        s = requests.Session()
        # Retry Step 1 with exponential backoff on 429 rate-limit responses
        r1 = None
...
def get_shared_token():
    ...
    # Try local/persisted token file first (avoids rate-limit on re-deploys)
    try:
        with open(TOKEN_FILE) as f:
            token = f.read().strip()
        if token:
        ...
        pass
    # Auto-generate via TOTP (retries built-in for 429)
    token, error = generate_token()
    if token:
        # Persist so next cold-start skips the login flow entirely
        try:
